...
// set true, show WAIT-ON screen when view open
window.viewWaitOn = false;
...
wingui.util.date.toPartialWeekString = function (date, isWeek) {
  var calendar = wingui.util.date.calendar;
...
wingui.util.grid.GridWrap = function (gridView) {
  ...
  this.init = function (regexp) {
    ...
    this.gridView.addCellStyle('uneditable ', {
      'background': '#f9f9f9',
      'readOnly': true,
      'editable': false
    }, true);
  ...
  this.initEntry = function (regexp, viewBuck, varBuck1, varDate1, varBuck2, varDate2, buck, endDate) {
    ...
    var isYear = false;
    var isDay = false;
//    console.log("viewBuck", viewBuck, " varBuck1:", varBuck1, "varDate1:",varDate1, "varBuck2:",varBuck2, "varDate2:", varDate2, "buck:",buck, "endDate:",endDate);
    for (var i = 0, n = this.dateColumnNames.length; i < n; i++) {
      var dateColumnName = this.dateColumnNames[i];
      var pattern = dateColumnName.match(regexp);
      if (!pattern) {
  ...
  this.setBucketHeaderText = function (isWeek) {
    ...
    for (var i = 0, n = this.dateColumnNames.length; i < n; i++) {
      ...
      if (this.yearDateColumnNames !== undefined && this.yearDateColumnNames.includes(dateColumnName)) {
        ...
        headerText = wingui.util.date.toPartialWeekString(date, isWeek);
      }
  ...
  this.setCellStyleMonthly = function (columnInfo) {
    for (var i = 0, n = this.monthlyDateColumnNames.length; i < n; i++) {
      var monthlyDateColumnName = this.monthlyDateColumnNames[i];

      var column = this.gridView.columnByName(monthlyDateColumnName);

      var stylesProperty = column.styles;
      Object.assign(stylesProperty, columnInfo.styles);
      this.gridView.setColumnProperty(monthlyDateColumnName, 'styles', stylesProperty);

      var headerProperty = column.header;
      Object.assign(headerProperty, columnInfo.header);
      this.gridView.setColumnProperty(monthlyDateColumnName, 'header', headerProperty);
    }
  }
  ...
  this.setCellStyleUneditable = function (fromDate, toDate, measureCodes, type) {
    ...
    for (var i = 0, n = this.dateColumnNames.length; i < n; i++) {
      ...
      if (this.monthlyDateColumnNames.includes(dateColumnName)) {
        date = wingui.util.date.toLastDateString(dateColumnName, this.regexp).replace(/[^\d]/g, '');
      } else {
      ...
      for (var j = 0, len = this.dataProvider.getRowCount(); j < len; j++) {
        ...
        this.gridView.setCellStyle(j, dateColumnName, 'uneditable');
        if (type === 'DTF') {
          var styleExceptCell = {
  ...
  this.addCellStyle = function (cellStyleObj) {
    if (!this.cellStyleObjs) {
      this.cellStyleObjs = [];
    }

    this.cellStyleObjs.push(cellStyleObj);
  };

  this.applyCellStyles = function () {
    if (!this.cellStyleObjs) {
      return;
    }

    var cellStyles = [];
    for (var i = 0, n = this.cellStyleObjs.length; i < n; i++) {
      cellStyles.push(this.cellStyleObjs[i].cellStyle);
    }
    this.gridView.addCellStyles(cellStyles);

    var itemRows = this.getRows();
    for (var item in itemRows) {
      var accountRows = itemRows[item];
      for (var account in accountRows) {
        var rows = accountRows[account];

        for (var i = 0, n = this.cellStyleObjs.length; i < n; i++) {
          var cellStyleObj = this.cellStyleObjs[i];

          var fieldNames = [];
          for (var j = 0, len = this.dateColumnNames.length; j < len; j++) {
            var fieldName = this.dateColumnNames[j];

            if (cellStyleObj.isConditionSatisfied(rows, fieldName)) {
              fieldNames.push(fieldName);
            }
          }

          if (fieldNames.length > 0) {
            this.gridView.setCellStyles(cellStyleObj.getDataRows(rows), fieldNames, cellStyleObj.cellStyle.id);
          }
        }
      }
    }
  ...
  this.addSummaryColumns = function (summaryInfo, totalSummaryInfos) {
    this.summaryColumn = new wingui.util.grid.SummaryColumn(this, this.gridView, this.dateColumnNames);
  ...
  this.addSummaryFooter = function (measureColumnName, groupMeasureName, totalMeasureNames) {
    this.summaryFooter = new wingui.util.grid.SummaryFooter(this.gridView, this.dateColumnNames, this.summaryColumn);
    this.summaryFooter.create(measureColumnName, groupMeasureName, totalMeasureNames);
  };
};

wingui.util.grid.SummaryColumn = function (gridWrap, gridView, dateColumnNames) {
  ...
  this.create = function (summaryInfo, totalSummaryInfos) {
    ...
    var firstColumn = this.gridView.columnByName(this.dateColumnNames[0]);
    ...
    for (var i = 0, len = this.summaryColumnKeys.length; i < len; i++) {
      ...
      for (var j = 0, n = columns.length; j < n; j++) {
        if (columns[j].name === targetColumnName) {
          ...
          this.gridView.addColumn(summaryColumn, null, i + j + 1);
          break;
        }
      }
    }
    ...
    if (this.totalSummaryInfos) {
      for (var i = 0, n = this.totalSummaryInfos.length; i < n; i++) {
        ...
        this.gridView.addColumn(summaryColumn, null, index);
      }
    }
...
wingui.util.grid.SummaryFooter = function (gridView, dateColumnNames, summaryColumn) {
  this.gridView = gridView;
  this.dataProvider = gridView.getDataSource();
  this.dateColumnNames = dateColumnNames;
  ...
  this.create = function (measureColumnName, groupMeasureName, totalMeasureNames) {
    ...
    this.gridView.setFooter({
      'count': this.measureNames.length,
      'height': 25 * this.measureNames.length
    });
    ...
    this.gridView.setColumnProperty(this.measureColumnName, 'footer', {
      text: this.measureNames.map(function (measureName) {
        return transLangKey(measureName);
      }),
      groupExpression: 'count',
      groupStyles: {
        prefix: transLangKey(this.groupMeasureName) + '(',
        suffix: ')'
      }
    });
    ...
    for (var i = 0, n = columnNames.length; i < n; i++) {
      this.gridView.setColumnProperty(columnNames[i], 'footer', {
        callback: function (column, footerIndex, gridView) {
          return summaryFooter.getSummary(footerIndex, column, gridView);
        },
        groupCallback: function (groupFooterItemindex, column) {
          return summaryFooter.getGroupSummary(groupFooterItemindex, column, gridView);
        }
      });
    }

    this.gridView.setStyles({ footer: { textAlignment: 'far' } });
  };
...
wingui.util.grid.filter = (function () {
  ...
  return {
    ...
    onFilteringChanged: function (gridView, column) {
      ...
      console.log(column)
      gridOnFilteringChanged(gridView, column);
...
//        console.log(filterNames);
    ...
    loadFilters: function (gridView, columnNames, excludeNames) {
      ...
      for (var i = 0, n = columnNames.length; i < n; i++) {
        ...
        var filterNames = gridView.prefInfo.filter(function (prefRow) {
          return prefRow.fldActiveYn && prefRow.crosstabItemCd === 'GROUP-VERTICAL-VALUES';
        }).map(function (prefRow) {
          return prefRow.fldApplyCd;
        });
